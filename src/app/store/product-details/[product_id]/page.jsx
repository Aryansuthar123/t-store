...
export default function ProductDetailsPage() {
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      ...
      setMessage("");
      try {
        const res = await fetch(`/api/products/${product_id}`, { cache: "no-store" });
        if (!res.ok) {
  ...
  return (
    <div className="max-w-7xl mx-auto px-12 py-10">
        ...
        <div className="flex-1">
          ...
          <div className="flex flex-wrap items-center gap-4 mt-5">
            ...
            <p className="text-lg font-medium">
              Total: ₹{(parseFloat(product.salePrice ?? product.price) || 0) * quantity}
            </p>
        ...
        {preview && (
          <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50">
            <button
              ...
              className="absolute top-5 right-5 text-white text-3xl font-bold">×</button>
            <Image
              src={preview}
              alt="preview"
              width={600}
      ...
      <div className="max-w-7xl mt-10 bg-white p-6 rounded-2xl  border border-pink-200">
          ...
          <div className="flex gap-1">
            {[1, 2, 3, 4, 5].map((star) => (
              <button
                ...
                <span className={star <= newRating ? "text-yellow-500" : "text-gray-300"}>★</span>
              </button>
            ))}
          </div>
        </div>
        ...
        <div className="mt-10 py-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            {relatedProducts.map((related) => (
              <div key={related._id} className="border rounded-xl p-4 hover:shadow-lg transition bg-white flex flex-col justify-between h-full">
